...
export function NovaCircleDash({ size = 36, dashScale = 1, className, color = "currentColor" }: Props) {
  const groupWidth = size * 2.97; // 6.5rem when circle is 35px
  const dash = Math.max(2, Math.round(size * 0.46 * dashScale)); // 16px when circle is 35px
